...
from template.DLPG_MDN_Template import DLPG_MDN_ARGS_Template
...
from utils.loss import DLPG_MDN_Loss
...
def main(args: DLPG_MDN_ARGS_Template):
    ### SEED ###
    ...
    if args.SAVE_RESULT_PATH is None:
        args.SAVE_RESULT_PATH    = Path(BASEDIR).parent/"DLPG_MDN"/args.runname/"results"    
            
    if args.SAVE_WEIGHT_PATH is None:
        args.SAVE_WEIGHT_PATH = Path(BASEDIR).parent/"DLPG_MDN"/args.runname/"weights"
    ...
    for iteration in pbar:
        ...
        if args.Training:
            if (iteration+1)%args.update_every==0 and iteration > args.batch_size:
                ...
                train_log_dictionary = DLPG_MDN_Loss(model, batch, normalizer)
        ...
        if (iteration)%args.eval_batch_size==0:
            ...
            test_log_dictionary = DLPG_MDN_Loss(model, batch, normalizer)
